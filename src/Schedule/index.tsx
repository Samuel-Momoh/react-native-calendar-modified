...
import { extractCalendarListProps, extractReservationListProps } from 'react-native-calendars/src/componentUpdater';
import { xdateToData, toMarkingFormat } from 'react-native-calendars/src/interface';
import { sameDate, sameMonth } from 'react-native-calendars/src/dateutils';
// @ts-expect-error
...
import styleConstructor from 'react-native-calendars/src/Schedule/style';
import WeekDaysNames from 'react-native-calendars/src/commons/WeekDaysNames';
import CalendarList, { CalendarListProps, CalendarListImperativeMethods } from 'react-native-calendars/src/calendar-list';
import ExpandableCalendar, { ExpandableCalendarProps } from 'react-native-calendars/src/expandableCalendar';
import WeekCalendar, { WeekCalendarProps } from 'react-native-calendars/src/expandableCalendar/WeekCalendar';
import ReservationList, { ReservationListProps } from 'react-native-calendars/src/Schedule/reservation-list';
...
export type ScheduleProps = CalendarListProps & ReservationListProps & ExpandableCalendarProps & WeekCalendarProps & {
 ...
 */
export default class Schedule extends Component<ScheduleProps, State> {
  ...
  componentDidUpdate(prevProps: ScheduleProps, prevState: State) {
  ...
  static getDerivedStateFromProps(nextProps: ScheduleProps) {
    if (nextProps.items) {
  ...
  loadReservations(props: ScheduleProps) {
  ...
  onArrowClick = (day: XDate) => {
    let date: XDate = new XDate(day)

    const withAnimation = sameMonth(date, this.state.selectedDay);
    this.calendar?.current?.scrollToDay(date, this.calendarOffset(), withAnimation);
...
dateChangeFnc = (day: XDate) => {
  ...
  renderCalendarList() {
    ...
    // console.log(this.state.selectedDay,"marking dats")
    return (
      <CalendarProvider
  ...
  render() {
    ...
    return (
      <>
        <Animated.View
          style={{ zIndex: 999, marginBottom: 40 }}
        >
